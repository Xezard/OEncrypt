...
export class PublicKeySetting implements PluginSetting {
  name: string
  description: string
  ...
  constructor(
    ...
    this.name = this.translationService.t('settings.public-key.name')
    this.description = this.translationService.t('settings.public-key.description')
  }
    ...
    setting.addButton((button) => {
      button.setButtonText(this.translationService.t('select')).onClick(() => {
        FileService.readFileFromFileSystem(SUPPORTED_KEY_FILE_EXTENSIONS, async (file, content) => {
          ...
          await this.state.set('publicKey', key)

          this.uiService.updatePGPKeyFileDisplay(setting, this.state.publicKey)
        })
      })
