...
export class DecryptNotePluginEvent implements PluginEvent {
  title: string
  icon = 'unlock'
  ...
  constructor(
    ...
    this.title = translationService.t('decryption.note')
  }
    ...
    return plugin.app.workspace.on('file-menu', (menu) => {
      ...
      menu.addItem((item) => {
        item.setTitle(this.title)
         .setIcon(this.icon)
         .onClick(async () => await this.encryptionService.decryptFile())
      })
    })
  }
}